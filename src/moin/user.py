...
class User:
    ...
    def avatar(self, size=30):
        ...
        email = self.email
        default = url_for("static", filename="img/circle-user-solid_blue.png")

        if not email:
            return default

        default = url_for("static", filename="img/circle-user-solid_blue.png", _external=True)
        email_encoded = email.lower().encode("utf-8")
        email_hash = hashlib.sha256(email_encoded).hexdigest()

        query_params = urlencode({"d": default, "w": str(size)})
